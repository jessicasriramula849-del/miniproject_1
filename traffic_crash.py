...
if selected == "Home":
    ...
    st.write("Traffic crash dataset contains valuable insights that can help improve road safety, optimize emergency response, and support policy decisions, to achieve these insights, I have analysed the traffic crash dataset csv file provided and extracted all the 15 query data outputs from the entire traffic crash dataset using VScode and MySQL queries, in this project I will discuss my findings and analysis of all the 15 query data outputs.")
    st.image("dataanalysis.jpg", caption="Jessica Sriramula")

if selected == "Queries":
    st.subheader("Analysis of all the 15 Query Data Outputs.")
    one = st.selectbox("Here 18 queries are displayed in the drop-down list as I have separated the 15 queries into further three queries for easier representation, please choose the query you want to display:", ["Query 1: top 5 most dangerous combinations of weather and crash type", 
                                                                        ...
                                                                        "Query 14: high risk time slot and the bucket that has the highest injury crashes", 
...
ORDER BY 
    ...
    if one == "Query 5: the top 5 primary causes of crashes during night time for crash hour >=18":
        st.write("Here we can see all the top 5 primary causes of crashes during night time for crash hour >=18 where the primary contributory cause unable to determine has 24500 total crashes for crash hour greater than or equal to eighteen.")
        query = """SELECT 
...
LIMIT 1;"""
    ...
    if one == "Query 14: high risk time slot and the bucket that has the highest injury crashes":
        st.write("Here we can see that after grouping all the time slots into morning,afternoon,evening and night, the high risk time slot and the bucket is night time slot as it has the highest number of injury crashes.")
        query = """SELECT 
    CASE 
...
LIMIT 3;"""
    ...
    if one == "Query 16: the year-over-year growth rate of crashes":
        st.write("Here we can see the year over year growth rate of the crashes from the years 2020 - 2026, where the highest year over year growth rate of crashes is 33% in the year 2024.")
        query = """
...
ORDER BY 
    ...
    if one == "Query 18: top 10 zones with highest crashes":
        st.write("Here we can see the top 10 zones of latitude and longitude with highest number of crashes.")
        query = """SELECT 
